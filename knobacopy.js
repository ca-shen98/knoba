...
export default defineComponent({
  ...
  async run({ steps, $ }) {
    async function getContentsBatch(qExternalIdsBatch, $) {
    ...
    async function getIngestedContentDataBatch(qExternalIdsBatch, $) {
    ...
    async function handlReceive(uqExternalIdsBatch, dqExternalIdsBatch, $) {
      ...
      const mKnobaIdsBatch = await Promise.all(uqExternalIdsBatch.concat(dqExternalIdsBatch)
      ...
      const mudKnobaIdContents = udKnobaIdContentsBatch
      ...
      const gudKnobaIdContents = Object.entries(mudKnobaIdContents)
        .reduce((gudKnobaIdContents,
          ...
          if (!uqExternalIds) {
            ...
            if (dqExternalIds) {
              const fExternalIds = new Set(JSON.parse(fetchedContent[mudKnobaId].metadata.external_ids));
    ...
    await handlReceive(steps.trigger.event.uqExternalIds ?? [], steps.trigger.event.dqExternalIds ?? [], this);
  },
})